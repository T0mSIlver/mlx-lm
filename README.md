...
- **Translate the checkpoint position after a cache hit** — supporting the same split-prompt behavior also requires adjusting how the next checkpoint is scheduled. In `mlx_lm`, checkpoint positions are interpreted relative to the prompt segment that still needs to be processed. This fork subtracts any already reused prefix before scheduling the next checkpoint, so repeated split-prompt requests keep re-saving the shared prefix.
