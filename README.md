...
- **Keep that shared-prefix checkpoint available alongside full-request caches** — supporting the split-prompt checkpoint above also requires a small cache-insertion change. On `upstream/main`, inserting a later trimmable cache entry deletes any shorter cache entry it encounters on the same token path. This fork preserves the shared-prefix checkpoint when later non-checkpoint caches are inserted, so the reusable prefix is still there on the next request.
- **Keep the checkpoint anchored to the shared prefix after a cache hit** — supporting the same split-prompt behavior also requires adjusting how the next checkpoint is scheduled. In `mlx_lm`, checkpoint positions are interpreted relative to the prompt segment that still needs to be processed. After a request has already reused the shared prefix, this fork subtracts that reused portion before scheduling the next checkpoint, so the saved checkpoint stays at the same shared-prefix boundary instead of drifting into the per-request tail.
